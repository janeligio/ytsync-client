...
export default function VideoQueue(props: VideoQueueProps) {
    ...
    const queueEl = queueData
        ? queueData.map((datum, index) => {
              const isPlaying = currentVideo === index ? true : false;
              return (
                  <VideoData
                      key={randomRoomNumber(10)}
                      index={index}
                      videoData={datum}
                      loadVideo={loadVideo}
    ...
    return (
        <div className="youtube-queue">
            <Container className="youtube-queue-container" fluid>
                {queueData.length > 0 ? queueEl : <QueueSkeleton />}
            </Container>
            <div style={{ padding: '0' }}>
...
function VideoData({
    ...
    const thumbnail = videoData.thumbnail ? (
        <Image fluid src={videoData.thumbnail.url} />
    ) : (
    ...
    return (
        <Row style={{ color: 'white', marginBottom: '0.25em' }}>
            <Col
                ...
                style={{ padding: 0, background: isPlaying ? 'var(--p)' : '' }}
                className="queue-element-container"
            >
                <div className="queue-element-index">
                    <small>{index + 1}</small>
                </div>
                <div className="queue-element-thumbnail">{thumbnail}</div>
                <div className="queue-element-info">
                    <h3 className="queue-element-title">{videoData.title}</h3>
                    <small className="queue-element-channel">
                        {videoData.channel}
                    </small>
                </div>
